...
from fastapi import APIRouter, Depends, File, HTTPException, Path, status, Security
...
async def get_groups_in_db(current_user: UserOutputDatabase = Security(get_current_active_user, scopes=[READ_PERMISSION])):
...
async def create_group(group_info: GroupInCreate, current_user: UserOutputDatabase = Security(get_current_active_user, scopes=[WRITE_PERMISSION])):
...
async def get_group_in_db(group_id: str, current_user: UserOutputDatabase = Security(get_current_active_user, scopes=[READ_PERMISSION])):
    """Get information of the number of samples per group loaded into the database."""
    group = await get_group(db, group_id)
    return group


@router.post("/groups/{group_id}/image", status_code=status.HTTP_200_OK, tags=["groups"])
async def update_image_for_group(group_id: str, image: bytes = File(...), current_user: UserOutputDatabase = Security(get_current_active_user, scopes=[WRITE_PERMISSION])):
    """Add an impage to a group."""
    # cast input information as group db object
    return {"id": group_id, "file_size": len(image)}


@router.put("/groups/{group_id}/sample", status_code=status.HTTP_200_OK, tags=["groups"])
