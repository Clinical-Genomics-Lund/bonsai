...
#    ),
@router.post("/cluster/{typing_method}/", status_code=status.HTTP_201_CREATED, tags=DEFAULT_TAGS)
async def cluster_samples(
    ...
    """
    try:
        profiles: TypingProfileOutput = await get_typing_profiles(db, clusterInput.sample_ids, typing_method.value)
    ...
    newick_tree: str = cluster_on_allele_profile(profiles, clusterInput.method, clusterInput.distance)
    return newick_tree 