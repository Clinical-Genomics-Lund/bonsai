...
from app.mimer import cgmlst_cluster_samples, get_sample_by_id, get_group_by_id, post_comment_to_sample, remove_comment_from_sample, TokenObject
...
def sample(sample_id):
    ...
    if group_id:
        ...
        for category, valid_genes in validated_genes.items():
            pred_res = next(iter([r for r in sample["phenotypeResult"] if r["type"] == category]))
            # filter genes based on the list of validated genes/ phenotypes for group
            if category.endswith("resistance"):
                filtered_genes = [res for res in pred_res["result"]["genes"] if _has_phenotype(res, valid_genes)]
                filtered_mutations = [res for res in pred_res["result"]["mutations"]  if _has_phenotype(res, valid_genes)]
                resistance = {phe 
                ...
                genes = [gene for gene in pred_res["result"].get("genes", []) if gene["name"] in valid_genes]
                mutations = [mut for mut in pred_res["result"].get("mutations", []) if mut["genes"] in valid_genes]
