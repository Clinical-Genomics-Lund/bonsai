...
function D3MSMetadataTable(tree, context_menu) {
  ...
  this.columns = [
    {
      id: "Selected",
      name: "<img src='tree/static/js/img/tick.png'>",
      field: "__selected",
      width: 30,
      maxWidth: 30,
      minWidth: 30,
    ...
    },
    {
      ...
      field: "id",
      width: 40,
      prop: { category: "numeric", group_num: 30, group_order: "standard" },
  ...
  $(this.grid.getHeaderRow()).delegate(
    ":input",
    "change keyup",
    function (e, args) {
  ...
  this.grid.selectionmodel.onSelectedRangesChanged.subscribe(
    function (e, args) {
      if (
        args[0].fromCell == args[0].toCell &&
        args[0].fromRow == args[0].toRow &&
        self.grid.selection[0][0] >= 0 &&
        self.grid.selection[1][0] >= 0
      ) {
        var range = {};
        [range.fromRow, range.toRow] =
          self.grid.selection[0][0] < self.grid.selection[1][0]
            ? [self.grid.selection[0][0], self.grid.selection[1][0]]
            : [self.grid.selection[1][0], self.grid.selection[0][0]];
        [range.fromCell, range.toCell] =
          self.grid.selection[0][1] < self.grid.selection[1][1]
            ? [self.grid.selection[0][1], self.grid.selection[1][1]]
            : [self.grid.selection[1][1], self.grid.selection[0][1]];
      ...
      if (
        args[0].fromCell == args[0].toCell &&
        self.grid.getColumns()[args[0].fromCell].field === "__selected"
      ) {
        var item = self.grid
          .getData()
          .getFilteredItems()
          .slice(args[0].fromRow, args[0].toRow + 1);
...
D3MSMetadataTable.prototype.selectItems = function (item, selectMode = "auto") {
  ...
  self.tree.force_nodes.filter(function (n) {
    if (involvedNodes[n.id]) {
      n.selected = selectMode;
    }
  });
  self.tree.clearSelection(true);
  self.tree._addHalos(
...
D3MSMetadataTable.prototype._setupDiv = function () {
  var self = this;
  var grid_html =
    "<div id = 'metadata-div' style='font-size:0.75em;width:700px;height:600px;position:absolute;top:10%;left:50%;z-index:2;background-color:#f1f1f1;display:none'>\
  ...
  $("#replace-confirm").click(function (e) {
    ...
    for (
      var col_id = self.grid.mouse_pos[1];
      col_id <= self.grid.mouse_pos[3];
      ++col_id
    ) {
      if (columns[col_id].editor) {
        var col = columns[col_id].field;
        for (
          var id = self.grid.mouse_pos[0];
          id <= self.grid.mouse_pos[2];
          id++
        ) {
          data[id][col] = val;
        }
  ...
  $("#metadata-filter").click(function (e) {
    ...
    var headerCell = $(self.grid.getHeaderRow()).find(
      ".l" + self.grid.getColumnIndex("Selected") + " :input",
    );
  ...
  $("#metadata-add-icon").click(function (e) {
    $("<div id ='metadata-add'></div>")
      .html(
        "Name of The New Column:<br> <input type='text' style='height:100%' id ='metadata-add-colname'>",
      )
      .dialog({
        modal: true,
        buttons: {
...
D3MSMetadataTable.prototype.updateMetadataTable = function () {
  ...
  for (var id in this.tree.metadata) {
    ...
    if (
      this.tree.node_map[d.__Node] &&
      (!this.tree.node_map[d.__Node].hypothetical ||
        this.tree.show_hypothetical_nodes)
    ) {
  ...
  this.grid.render();
  $(".slick-headerrow-column").attr(
    "title",
    "Type in to filter records on the columns",
  );
};
...
D3MSMetadataTable.prototype.sendToMicroReact = function (
  ...
  $.ajax({
    ...
    .fail(function (e, ui) {
      callback();
      var microDiag = $(
        "<div id='micro-dialog' title='Failed with unknown reason'.>Failed with unknown reason<br></div>",
      );
      microDiag.appendTo($("body"));
      var microLink = microDiag.find("#micro-link");
      microLink.attr("href", e);
