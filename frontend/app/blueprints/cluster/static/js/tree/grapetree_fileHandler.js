...
function getHelpBox(help_item) {
  if (help_item in help_text) {
    document.getElementById("welcome-div-text").innerHTML =
      help_text[help_item];
  } else {
    document.getElementById("welcome-div-text").innerHTML =
      "<b>No Help text available for this feature</b>";
  }
  $("#welcome-div").toggle();
}
      ...
      success: function (tree) {
        ...
        if (the_tree && metadata) {
          $.ajax({
            type: "GET",
            url:
              "https://enterobase.warwick.ac.uk/grapetree_remote/" + metadata,
            headers: { "X-Requested-With": "XMLHttpRequest" },
...
function distributeFile(text, filename) {
  ...
  if (
    head_line.startsWith(">") ||
    (head_line.startsWith("#") &&
      !head_line.toUpperCase().startsWith("#NEXUS")) ||
    (head_line.indexOf("\t") >= 0 && !the_tree)
  ) {
    if (cannot_connect) {
    ...
    (head_line.indexOf(",") >= 0 || head_line.indexOf("\t") >= 0) &&
    !head_line.startsWith("(") &&
    !head_line.startsWith("[") &&
    !head_line.startsWith(" ") &&
    !head_line.startsWith("{")
...
function saveTextAsFile(text, suggested_name) {
  ...
  var notification =
    "<p style='font-size:10'> Some browsers allow you to select a folder after you click the 'Save' Button.<br> Whereas many browsers, such as Chrome, by default save the downloaded file into:<br><b>Windows</b>: <code>'&#92;Users&#92;&lt;username&gt;&#92;Downloads'</code><br><b>Mac</b>: <code>'/Users/&lt;username&gt;/Downloads'</code><br><b>Linux</b>: <code>'home/&lt;username&gt;/Downloads'</code><br></p>";
  var filenamebar = $(
    "<input title='Suffix will be automatically added in some browsers if you have got a file with the same name.' type='text' id ='filename' value='" +
      suggested_name +
      "'>",
  );
  ...
  try {
    $.ajax({
      ...
      .done(function (result) {
        ...
        var diag = $(
          '<div id="send-to-tree" title="Estimated memory / time usage"></div>',
        ).appendTo($("body"));

        diag.append(
          "<p><label>Memory (MB): </label>" +
            Math.round(sys_info.memory / 1024 / 1024) +
            "</p>",
        );
        var deltaTime = new Date() - this.startTime;
        diag.append(
          "<p><label>Time (minutes): </label>" +
            Math.round((sys_info.time + deltaTime / 1000) / 6) / 10 +
            "</p>",
        );
        diag.append(
          "<p>Numbers are raw estimates and may be different from your system. </p>",
        );
        if (!sys_info.affordable) {
          diag.append(
            "<p><label style='font-color:red'>WARNING: More memory are required than you current have. The tree may still be calculated because some system can automatically compress the memory. However the calculation will take more time and may crash! </label></p>",
          );
        }
      ...
      .fail(function (jqXHR, textStatus) {
        if (jqXHR.status == 405 || jqXHR.status == 404) {
          loadFailed(
            "Cannot reach the backend. Please download a FREE standalone version from https://github.com/achtman-lab/GrapeTree/",
          );
        } else {
          console.log(textStatus);
          loadFailed(
            "There server returned an error. Is the profile file in the right format?",
          );
        }
      });
  } catch (e) {
  ...
  try {
    $.ajax({
      ...
      .done(function (result) {
        ...
        $("#headertag").text(
          $("#headertag").text() + " (" + $("#method-select").val() + ")",
        );
        loadMSTree(tree_raw);
      })
      .fail(function (jqXHR, textStatus) {
