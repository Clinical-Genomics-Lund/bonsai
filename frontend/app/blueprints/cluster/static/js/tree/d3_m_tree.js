...
function D3MSTree(element_id, data, callback, height, width) {
  ...
  this.hide_link_length = data["hide_link_length"]
    ? data["hide_link_length"]
    : 10000;
  this.show_link_labels = false;
  this.show_hypothetical_nodes = false;

  this.distance_scale = d3.scale
    .linear()
    .domain([0, this.max_link_distance])
  ...
  this.force_drag = this.d3_force_directed_graph
    .drag()
  ...
  if (data["nexus"] || data["nwk"]) {
    var root = data["nexus"]
      ? this.parseNexus(data["nexus"])
      : this.parseNewick(data["nwk"]);
    if (
      data["layout_data"] &&
      data["layout_data"].nodes_links &&
      data["layout_data"].nodes_links.backup
    ) {
  ...
  if (callback) {
    callback(
      this,
      this.original_nodes
        ? "Collapsing Nodes:" + this.original_nodes.length
        : "Collapsing Nodes.",
    );
  }

  this._collapseNodes(0);
  this.original_node_positions = this.greedy_layout(
    this.force_nodes,
    this.max_link_scale,
    this.base_node_size,
  );

  var positions = data["layout_data"]
    ? data["layout_data"]["node_positions"]
    : null;

  var to_collapse = (this.node_collapsed_value = 0);
  this.manual_collapsing = {};
  if (positions && data["layout_data"]["nodes_links"]) {
    to_collapse = this.node_collapsed_value =
      data["layout_data"]["nodes_links"]["node_collapsed_value"];
    this.manual_collapsing = data["layout_data"]["nodes_links"][
      "manual_collapsing"
    ]
      ? data["layout_data"]["nodes_links"]["manual_collapsing"]
      : {};
  } else if (this.original_links.length > 20000) {
    var link_distances = this.original_links
      .map(function (l) {
        return l.distance;
      })
      .sort(function (n1, n2) {
    ...
    if (to_collapse < link_distances[20000]) {
      ...
      alert(
        "Too many nodes. Branches <= " +
          to_collapse +
          " are collapsed in initial layout. You can uncollapse them later. ",
      );
    }
...
D3MSTree.prototype.greedy_layout = function (nodes, link_scale, node_size) {
  ...
  var min_radial =
    (max_radial / link_scale) * (2 * Math.pow(node.size, this.size_power));
    ...
    for (var ite = 0; ite < 11; ite++) {
      ...
      for (var id = nodes.length - 1; id >= 0; id--) {
        ...
        for (var jd in node.children) {
          ...
          if (
            Math.cos(Math.PI - child.des_span[1]) >
            child.des_span[0] / self_radial
          ) {
            if (child.des_span[0] < self_radial) {
              var span = [
                self_radial,
                Math.asin(child.des_span[0] / self_radial),
              ];
            }
          } else {
            var span = to_Polar(to_Cartesian(child.des_span), [
              -self_radial,
              0,
            ]);
        ...
        if (node.des_span[1] > max_span[1]) max_span = [[id], node.des_span[1]];
        else if (node.des_span[1] == max_span[1]) max_span[0].push(id);
  ...
  for (var id = 0; id < nodes.length; ++id) {
    ...
    if (id > 0) {
      var initial_angle = -node.des_angle,
        gap = 0;
    } else {
      var initial_angle = 0,
        gap = Math.max(0, (Math.PI - node.des_angle) / node.children.length);
    }

    if (node.children) {
      if (node.children.length > 2) {
        node.children.sort(function (c1, c2) {
          return hashCode(c1.id) - hashCode(c2.id);
        });
      }
      for (var jd = 0; jd < node.children.length; ++jd) {
        ...
        child.polar = [
          child.link.value + self_radial,
          initial_angle + child.self_span[1] + min_angle + node.polar[1],
        ];
    ...
    delete node.spacing,
      delete node.des_angle,
      delete node.des_span,
      delete node.polar,
      delete node.coordinates;
  }
  return coordinates;
};
...
D3MSTree.prototype._start = function (callback, layout_data) {
  ...
  this.link_elements = this.canvas
    .selectAll("g.link")
    .data(this.force_links, function (it) {
  ...
  this.node_elements = this.canvas
    .selectAll(".node")
    .data(this.force_nodes, function (it) {
      return it.id;
    });

  //nodes
  this.node_elements.exit().remove();
  var new_node_elements = this.node_elements
    .enter()
    .append("g")
 ...
 */
D3MSTree.prototype.collapseNodes = function (
  max_distance,
  keep_current_layout,
) {
...
D3MSTree.prototype._collapseNodes = function (max_distance, layout, redraw) {
  ...
  if (this.original_links.length > 50000) {
    this.original_links.sort(function (n1, n2) {
  ...
  if (
    this.node_collapsed_value <= 0 &&
    !this.manual_collapsing_value &&
    layout
  ) {
    this.force_nodes.forEach(function (node) {
      layout[node.id] = self.original_node_positions[node.id] = [
        node.x,
        node.y,
      ];
    });
  ...
  if (
    ...
    redraw ||
    max_distance < this.node_collapsed_value
  ) {
    this.clearSelection();
    this.hypo_record = {};
  ...
  for (var index = this.force_links.length - 1; index >= 0; index--) {
    ...
    if (
      !l ||
      (l.value > max_distance && to_collapse[l.source.id] !== 2) ||
      (l.value && to_collapse[l.source.id] === 1)
    )
      continue;
  ...
  for (var id in this.force_nodes) {
    ...
    if (node.parent && this.hypo_record[node.parent.id] !== grp) {
      new_force_nodes[grp].parent =
        new_force_nodes[this.hypo_record[node.parent.id]];
    ...
    if (node.children) {
      var children_ids = node.children
        .filter(function (c) {
  ...
  var new_force_links = this.force_links
    .map(function (l) {
  ...
  var new_force_nodes = this.force_nodes
    .map(function (n) {
...
D3MSTree.prototype.setLayout = function (layout_data) {
  ...
  if (layout_data["nodes_links"]) {
    ...
    this.link_font_size = data["link_font_size"]
      ? data["link_font_size"]
      : this.link_font_size;
    this.distance_scale = d3.scale
      .linear()
    ...
    this.node_font_size = data["node_font_size"]
      ? data["node_font_size"]
      : this.node_font_size;
    this.show_individual_segments = data["show_individual_segments"];
    this.show_node_labels = data["show_node_labels"]
      ? data["show_node_labels"]
      : false;
    this.hide_link_length = data["hide_link_length"]
      ? data["hide_link_length"]
      : this.hide_link_length;
    this.custom_colours = data["custom_colours"]
      ? data["custom_colours"]
      : this.custom_colours;
    this.color_schemes.custom = data.custom_color_scheme
      ? data.custom_color_scheme
      : this.color_schemes.custom;
...
D3MSTree.prototype._drawLinks = function () {
  ...
  this.link_elements
    ...
    .attr("stroke-dasharray", function (it) {
      return self.max_link_length && it.value > self.max_link_length
        ? "3,5"
        : "";
    })
  ...
  this.link_elements
    ...
    .attr("font-size", this.link_font_size);
};
...
D3MSTree.prototype.toggleHypotheticalNodes = function () {
  ...
  for (var i in this.displayChangedListeners) {
    this.displayChangedListeners[i](
      "show_hypothetical_nodes",
      this.show_hypothetical_nodes,
    );
  }
  ...
  this.node_elements
    ...
    .attr("fill", function (it) {
      return self.category_colours[it.data.type]
        ? self.category_colours[it.data.type]
        : self.default_colour;
    });
  this.node_elements
    .selectAll(".halo")
    .attr("d", function (d) {
      ...
      var arc = d3.svg
        .arc()
        .innerRadius(r)
        .outerRadius(r + d.halo_thickness)
        .startAngle(0)
        .endAngle(2 * Math.PI);
      return arc();
    })
...
D3MSTree.prototype.delOtherNodes = function (nodes) {
  ...
  nodes.forEach(function (n) {
    self.grouped_nodes[n].forEach(function (x) {
      nn[x] = 1;
    });
  });
  this.delNodes(
    this.original_nodes.filter(function (n) {
      return !nn[n];
    }),
  );
};
...
D3MSTree.prototype.undeleteNodes = function () {
  ...
  if (self.backup) {
    ...
    self.manual_collapsing = JSON.parse(
      JSON.stringify(self.backup.manual_collapsing),
    );
    var selected = {};
    self.force_nodes
      .filter(function (n) {
    ...
    this._start(null, {
      node_positions: self.backup.positions,
      scale: this.scale,
      translate: this.translate,
    });
    self.backup = null;
...
D3MSTree.prototype.delNodes = function (nodes) {
  ...
  if (!self.backup.links)
    self.backup.links = JSON.parse(JSON.stringify(self.original_links));
  if (!self.backup.nodes)
    self.backup.nodes = JSON.parse(JSON.stringify(self.original_nodes));
  if (self.backup.nodes.length === self.original_nodes.length) {
    self.backup.node_collapsed_value = this.node_collapsed_value;
    self.backup.manual_collapsing = JSON.parse(
      JSON.stringify(self.manual_collapsing),
    );
    self.backup.original_positions = JSON.parse(
      JSON.stringify(self.original_node_positions),
    );
  ...
  this.max_link_distance = self.original_links
    .map(function (l) {
      return l.distance;
    })
...
D3MSTree.prototype._getPieData = function (d, category) {
  ...
  if (results.length == 0) {
    results = [
      {
        value: 1,
        type: this.metadata[d.id][category]
          ? this.metadata[d.id][category]
          : "missing",
        idx: d.id,
      },
    ];
...
D3MSTree.prototype._addLinks = function (links, ids) {
  ...
  this.link_distances = {};
  this.max_link_distance = new_links
    .map(function (l) {
      return l.value;
    })
    .reduce(function (v1, v2) {
...
D3MSTree.prototype._setLinkDistance = function (strict) {
  var self = this;
  var min_link_distance = Math.min.apply(
    Math,
    this.force_links.map(function (d) {
      return d.original_value;
    }),
  );
  var max_link_distance = this.max_link_distance;
  var log_adjust1 = Math.log(self.distance_scale(min_link_distance)) - 1.0;
  var log_adjust2 =
    self.distance_scale(max_link_distance) /
    (Math.log(self.distance_scale(max_link_distance)) - log_adjust1);
  this.link_elements.each(function (d) {
    ...
    if (self.log_link_scale) {
      length =
        (Math.log(self.distance_scale(line_len)) - log_adjust1) * log_adjust2 +
        length;
    } else {
...
D3MSTree.prototype._centerGraph = function () {
  var maxX = this.force_nodes[0].x,
    minX = this.force_nodes[0].x;
  var maxY = this.force_nodes[0].y,
    minY = this.force_nodes[0].y;
  ...
  var [newX, newY] = [
    ...
    ((this.height * 1.0) / scale - hdiff) / 2.0,
  ];
...
D3MSTree.prototype.clearSelection = function (pervasive) {
  if (!pervasive) {
    this.force_nodes.filter(function (node) {
      node.selected = false;
...
D3MSTree.prototype._showLinkLabels = function () {
  ...
  this.link_elements
    ...
    .style("opacity", "0.9")
    .text(function (it) {
      return it.original_value >= 1
        ? Math.floor(it.original_value)
        : it.original_value.toPrecision(2);
    });
  this.link_elements
    .append("text")
    .attr("class", "distance-label")
...
D3MSTree.prototype._nodeSizeAltered = function () {
  ...
  for (var i in this.force_links) {
    var link = this.force_links[i];
    var prev_length =
      self.previous_node_radii[link.source.id] +
      self.previous_node_radii[link.target.id];
    var current_length =
      self.node_radii[link.source.id] + self.node_radii[link.target.id];
...
D3MSTree.prototype._addHalos = function (filter_function, thickness, colour) {
  ...
  self.node_elements.sort(function (a, b) {
    if (a.halo_thickness) {
      if (b.halo_thickness) {
        return a.halo_thickness == b.halo_thickness
          ? 0
          : a.halo_thickness > b.halo_thickness
            ? -1
            : 1;
      } else {
        return 1;
...
D3MSTree.prototype.fixAllNodes = function (resetLength) {
  ...
  for (var ii in this.force_links) {
    ...
    var value =
      link_distance -
      this.node_radii[link.source.id] -
      this.node_radii[link.target.id];
...
D3MSTree.prototype.brushEnded = function (extent) {
  ...
  if (
    (extent[0][0] - extent[1][0]) * (extent[0][0] - extent[1][0]) +
      (extent[0][1] - extent[1][1]) * (extent[0][1] - extent[1][1]) <
    4
  ) {
    var selected_nodes = this.node_elements.filter(function (d) {
      r = self.node_radii[d.id] ? self.node_radii[d.id] : 3;
      return (
        (d.x - extent[0][0]) * (d.x - extent[0][0]) +
          (d.y - extent[0][1]) * (d.y - extent[0][1]) <=
        r * r
      );
    });
  ...
  var not_in_selection = selected_nodes.filter(function (d) {
    return !d.selected;
  });
  if (not_in_selection[0].length > 0) {
    not_in_selection.filter(function (d) {
      d.selected = true;
    });
  } else {
    selected_nodes.filter(function (d) {
      delete d.selected;
      delete node.halo_colour;
      delete node.halo_thickness;
    });
