...
function D3MSTreeContextMenu(tree, meta_grid, hide_tree_functions) {
  ...
  $("#context-menu").remove();
  var context_html =
    "<div id='context-menu' style='display:none;position:fixed;width:150px;z-index:4;background-color:#ffffff;border:1px solid #f2f2f2'> \
...
D3MSTreeContextMenu.prototype._init = function () {
    ...
    .change(function (e) {
      if (e.target.id === "color-colname") {
        ...
        if (!self.tree.metadata_info[colname]) {
          ...
          $("#allowed-color").show();
          $("#context-menu").height(
            $("#context-menu").height() + $("#allowed-color").height(),
          );
        }
      } else {
        var category = e.target.closest("#myGrid-menu")
          ? $("#hover-colname").val()
          : self.tree.display_category;
  ...
  $(".colorscheme").click(function (e, ui) {
    if (e.pageY == 0 && e.pageX == 0) {
      ...
      if (value === "custom") {
        var ccs_div = $(
          '<div title="Customize a color scheme" style="width:20em; height:22em" id="custom_color_scheme"></div>',
        ).appendTo($("body"));
        ccs_div.append(
          '<div style="margin-bottom:5px">Paste color codes into the text box. <br>One color per line. <br> The colors can be in their names, <br>HEX codes or other HTML compatible codes.<br> Find the details in <a href="https://www.w3schools.com/html/html_colors.asp" target="_blank">THIS LINK</a></div>',
        );
        ccs_div.data("data", self.tree.color_schemes);

        var colors = self.tree.color_schemes.custom;
        var legend_text = $(
          '<div style="width:12em;height:20em;resize:none;overflow:auto;white-space:nowrap;float: left;" readonly id="legend_text" ></div>',
        )
          .appendTo(ccs_div)
          .html(
            d3
              ...
              .map(function (d) {
                return (
                  '<div style="float:left; width:0.9em; height:0.9em; margin-right:0.1em; background:' +
                  d.group_colour +
                  '"></div>' +
                  d.group
                );
        ...
        var ccs_text = $(
          '<textarea style="width:8em;height:20em;resize:none;overflow:auto;white-space:nowrap;float: left;" id="ccs_text" ></textarea>',
        ).appendTo(ccs_div);
  ...
  $(".select-group").click(function (e) {
    ...
    var node_ids = {};
    Object.values(self.tree.metadata)
      .filter(function (d) {
  ...
  $("#go-left").click(function (e) {
    var row = self.meta_grid.grid.getActiveCell()
      ? self.meta_grid.grid.getActiveCell().row
      : 0;
...
D3MSTreeContextMenu.prototype._fill_metadata_option = function (source) {
  if (!source) source = {};
  $("#group-num-input")
    .val(source.category_num ? source.category_num : 30)
...
D3MSTreeContextMenu.prototype._trigger_context = function (target, e) {
  ...
  if (target == "mst-svg") {
    ...
    if (category != "nothing") {
      if (!this.tree.metadata_info[category]) {
        this.tree.metadata_info[category] = {
          category_num: 30,
          coltype: "character",
          grouptype: "size",
    ...
    try {
      $(".select-group").data("group", [
        category,
        d3.select(e.target).data()[0].real_group,
      ]);
      $(".select-group").show();
    } catch (e) {
      $(".select-group").hide();
    ...
    if (!colname) {
      if (self.meta_grid.grid.getCellFromEvent(e)) {
        colname =
          self.meta_grid.grid.getColumns()[
            self.meta_grid.grid.getCellFromEvent(e).cell
          ].field;
      } else {
  ...
  if (cntx.height() + cntx.offset().top + 10 > $("body").height()) {
    cntx.css(
      "top",
      Math.max($("body").offset().top, $("body").height() - cntx.height() - 10),
    );
  }
  if (cntx.width() + cntx.offset().left > $("body").width()) {
    cntx.css(
      "left",
      Math.max($("body").offset().left, $("body").width() - cntx.width()),
    );
  }
};
