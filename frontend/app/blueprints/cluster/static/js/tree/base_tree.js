                ...
                if (t == TokenTypes.QuotedString) {
                  ...
                  if (t == 6 || t == 9) {
                    if (nx.current_comment) {
                      ...
                      var info = nx.current_comment.substring(
                        1,
                        nx.current_comment.length - 1,
                      );
...
function D3BaseTree(element_id, metadata, height, width) {
  var self = this;
  this.legend_div = $("<div>")
    .css({ position: "absolute", "overflow-x": "hidden" })
  ...
  this.color_schemes = {
    category: d3.scale
      .category20()
      .range()
      .concat(d3.scale.category20b().range(), d3.scale.category20c().range(), [
        "#FFFF00",
        "#1CE6FF",
        "#FF34FF",
        "#FF4A46",
  ...
  var x_zoom_scale = d3.scale
    .linear()
    .domain([0, this.width])
    .range([0, this.width]);
  var y_zoom_scale = d3.scale
    .linear()
    .domain([0, this.height])
    .range([0, this.height]);
  this.zoom = d3.behavior.zoom().x(x_zoom_scale).y(y_zoom_scale);

  this.background_rect = this.svg
    .append("rect")
...
D3BaseTree.prototype.readNexusFile = function (text) {
  ...
  return {
    root: root,
    translate: nexus.treesblock.translate,
    node_information: nexus.node_information,
  };
};
...
D3BaseTree.prototype._changeCategory = function (category) {
  ...
  if (!this.metadata_info[category].coltype)
    this.metadata_info[category].coltype = "character";
  if (!this.metadata_info[category].grouptype)
    this.metadata_info[category].grouptype = "size";
  if (!this.metadata_info[category].colorscheme)
    this.metadata_info[category].colorscheme = "category";
  if (!this.metadata_info[category].minnum)
    this.metadata_info[category].minnum = 0;
  if (!this.metadata_info[category].category_num)
    this.metadata_info[category].category_num = 30;
  ...
  this.category_colours = {};
  var color_num =
    cat_count_list.length > category_num ? category_num : cat_count_list.length;
  ...
  for (var colour_count in cat_count_list) {
    ...
    if (cust_col && cust_col[val]) {
      ...
      cat_count_list[colour_count][2] = this.category_colours[val] =
        auto_col[colour_count];
    }
  }
...
D3BaseTree.prototype._updateScaleDivSize = function (scaleLength) {
  ...
  var scaleValue =
    digit > 0
      ? scaleValue.toFixed(digit)
      : Math.round(scaleValue / Math.pow(10, -digit)) * Math.pow(10, -digit);
...
D3BaseTree.prototype.updateLegend = function (title, ordered_groups) {
  ...
  var others = 0;
  var legend_data = ordered_groups
...
D3BaseTree.prototype.getSVG = function () {
  ...
  if (the_tree.legend_div.css("display") === "block") {
    ...
    leg.attr(
      "transform",
      "translate(" + ori_pos.left + "," + ori_pos.top + ")",
    );
  }
